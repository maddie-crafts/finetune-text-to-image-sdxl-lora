...
def main(args):
    ...
    # Only train the additional adapter LoRA layers
    vae.requires_grad_(False)
    text_encoder_one.requires_grad_(False)
    text_encoder_two.requires_grad_(False)
    unet.requires_grad_(False)

    # Convert non-trainable model parts (like VAE, and text/unet layers without LoRA) to half-precision
    # to save memory. Since these parts aren't being trained—just used for inference—they don't need full precision.
    ...
    # Now add new LoRA weights to the attention layers
    ...
    # The text encoder comes from huggingface transformers, attach adapters to it.
    ...
    # Initialize the trackers used, and also store our configuration.
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # Sample noise that it's going to be added to the latents
                ...
                if args.snr_gamma is None:
                    ...
                    # Compute the SNR and the loss weights
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt and args.num_validation_images > 0:
            ...
            logger.info(f"Generated {len(images)} images with prompt: {args.validation_prompt}.")
        else:
            logger.info("No validation prompt provided, skipping final validation.")
